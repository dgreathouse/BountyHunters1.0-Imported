...
public class AutoSourceMidNote extends SequentialCommandGroup {
  /** Creates a new AutoBlueLeft. */
  public AutoSourceMidNote(DrivetrainSubsystem _drive, ShooterSubsystem _shooter, IntakeSubsystem _intake) {
    ...
    addCommands(
      ...
      new AutoDriveOdometry(_drive, new Pose2d(0.66,0 * GD.G_AllianceSign,new Rotation2d(Math.toDegrees(0))), 1.85),      // Drive to shot
      ...
      new AutoDriveOdometry(_drive, new Pose2d(3.6,-2.27 * GD.G_AllianceSign,new Rotation2d(Math.toDegrees(0))), 1.85),      // Drive to Mid area
      new InstantCommand(_intake::spinOn),                                                                          // Turn on the intake
      new AutoDriveOdometry(_drive, new Pose2d(7.36,0 * GD.G_AllianceSign,new Rotation2d(Math.toDegrees(0))), 1.85),      // Drive to Note
      new AutoDriveDelayCommand(_drive, 0.5),                                                                       // Delay for Note to be grabbed
      new AutoDriveOdometry(_drive, new Pose2d(3.6,-2.27 * GD.G_AllianceSign,new Rotation2d(Math.toDegrees(0))), 1.85),      // Drive to Mid area
      new InstantCommand(_intake::spinOff),                                                                         // Turn off intake
      new ShooterSetShotCommand(_shooter, ShooterState.PODIUM),                                                     // Set Shooter Speed HIGH
      new FlipperSetCommand(_shooter, FlipperStates.PRELOAD),                                                       // Preload flippers
      new AutoDriveOdometry(_drive, new Pose2d(1.2,0 * GD.G_AllianceSign,new Rotation2d(Math.toDegrees(0))), 1.85),      // Drive to Shot
      new AutoDriveRotateCommand(_drive, -53 * GD.G_AllianceSign, 1),                                               // Rotate to Speaker
      new FlipperSetCommand(_shooter, FlipperStates.SHOOT),                                                         // Shoot Note
